...
// initial release by Sylvain GARNAVAULT - 2016/03/13
	...
	class ADS8688 {
		public:
            ADS8688();
            bool init(SPIClass *theSPI, uint8_t cs);
            ADS8688(SPIClass *theSPI);                                    // instantiate with PIN10 as CS
			//ADS8688(SPIClass *theSPI, uint8_t cs);        // instantiate with custom CS pin
